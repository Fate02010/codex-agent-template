...
| 步骤 | 步骤名称 | 完成判定条件 | 续执起点 |
|---|---|---|---|
| 步骤 0 | P0 Gate | 最小输入集合可读且报告输出路径可写 | 步骤 1 |
| 步骤 1 | 提取全部标识符 | 所有文档的标识符已收集完毕 | 步骤 2 |
| 步骤 2 | 校验调研→需求覆盖 | 调研→需求覆盖检查已完成，孤立调研项已列出 | 步骤 2.5 |
| 步骤 2.5 | 校验 PRD_RAW.md 结构完整性 | PRD_RAW.md 结构检查已完成（或文件不存在跳过） | 步骤 2.6 |
| 步骤 2.6 | 校验 PRD_RECTIFIED.md 冻结可用性 | PRD_RECTIFIED.md 冻结可用性检查已完成（或文件不存在跳过） | 步骤 3 |
| 步骤 3 | 校验需求→接口覆盖 | 需求→接口覆盖检查已完成 | 步骤 4 |
| 步骤 4 | 校验接口→用例覆盖 | 接口→用例覆盖检查已完成 | 步骤 4.5 |
| 步骤 4.5 | 校验需求→页面设计覆盖 | 页面设计覆盖检查已完成（或 02-design 文档不存在跳过） | 步骤 5 |
| 步骤 5 | 校验用例→需求/接口引用有效性 | 用例引用有效性检查已完成 | 步骤 6 |
| 步骤 6 | 校验数据模型覆盖 | 数据模型覆盖检查已完成 | 步骤 7 |
| 步骤 7 | 校验 CR→迭代纳入一致性 | CR 纳入一致性检查已完成（或迭代文档不存在跳过） | 步骤 8 |
| 步骤 8 | 校验版本基线完整性 | 版本基线完整性检查已完成（或迭代文档不存在跳过） | 步骤 9 |
| 步骤 9 | 校验复盘闭环一致性 | 复盘闭环一致性检查已完成（或复盘文档不存在跳过） | 步骤 10 |
| 步骤 10 | 校验文档元数据完整性 | 元数据完整性检查已完成 | 步骤 11 |
| 步骤 11 | 校验缺陷引用 | 缺陷引用检查已完成（或 DEFECT_LOG.md 不存在跳过） | 步骤 12 |
| 步骤 12 | 生成 DOC_CHECK_REPORT.md | `docs/DOC_CHECK_REPORT.md` 文件存在且 verdict 字段有值 | 步骤 13 |
| 步骤 13 | 提示后续动作 | 已完成后续动作提示 | — |
