...
本 Skill 输出为终端结论（并行判定、任务拆分、收口清单），不写入文档文件。断点恢复以"对话上下文中是否已有并行判定结论"为依据。

| 步骤 | 步骤名称 | 完成判定条件 | 续执起点 |
|---|---|---|---|
| 步骤 1 | 并行可行性判定 | 对话上下文中已有 `PARALLEL_ENABLED`/`PARALLEL_DISABLED` 结论 | 步骤 2 |
| 步骤 2 | 任务拆分与所有权 | 对话上下文中已有 backend/frontend 任务包与共享依赖清单 | 步骤 3 |
| 步骤 3 | 并行执行约束 | 对话上下文中已有并行约束说明（禁止修改对端文件等） | 步骤 4 |
| 步骤 4 | 统一收口 | 对话上下文中已有收口清单（联调项、TC 映射校验项、准出检查项） | — |

### 默认恢复原则（兜底）

1. 若无法从上下文判断任何步骤已完成，从步骤 1 全量重新执行。
2. 本 Skill 无持久化产物，重新执行成本低，建议优先全量执行而非尝试续执。
3. 若 `PARALLEL_DISABLED` 结论已输出，无需续执，直接回退到单线 `dev-implement`。
