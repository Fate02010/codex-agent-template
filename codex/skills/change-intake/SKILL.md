...
## 目标
统一登记增量需求、缺陷转需求和优化项，形成可供迭代规划直接消费的 CR 基线。
...
|---|---|---|---|---|---|---|---|
| CR-NNN | [变更标题] | [资料名称] | 新增功能/规则变更/体验优化/缺陷转需求/需求删除 | P0/P1/P2/P3 | 模块列表 | 待分析/已批准/已拒绝/已取消 | 未纳入/ITER-NNN |

## 2. 变更明细

### CR-NNN: [变更标题]
- **变更类型**：新增功能 / 规则变更 / 体验优化 / 缺陷转需求 / 需求删除
- **来源**：[资料名称/会议记录/变更单编号]
- **优先级**：P0 / P1 / P2 / P3
- **描述**：变更的具体内容
...
- **验收标准**：
  ...
  2. 验收条件 2
- **状态**：待分析 / 已批准 / 已拒绝 / 已取消
- **纳入迭代**：未纳入（由 `iteration-plan` 回写为 `ITER-NNN`）

## 异常与边界
- 已批准但尚未排期的 CR，`纳入迭代` 必须保持为“未纳入”
- 被拒绝或取消的 CR 保留记录，不得物理删除

## 完成检查
- [ ] 每个变更事项都有 CR 编号
- [ ] 每个事项都登记来源和状态
- [ ] 已批准 CR 的“纳入迭代”状态可追溯
- [ ] 可直接输入 `CHANGE_IMPACT.md`
...
## 目标
分析每个 CR 对需求、设计、代码、测试和发布时间的影响，为迭代纳入决策提供依据。
...
- `change-intake` 阶段默认将所有新增 CR 的`纳入迭代`设为"未纳入"
...
- `纳入迭代` 字段仅由 `iteration-plan` 更新，`change-intake` 只负责初始化为"未纳入"
...
| 步骤 | 步骤名称 | 完成判定条件 | 续执起点 |
|---|---|---|---|
| 步骤 1 | 读取现有基线 | 已读取 `PRD_RECTIFIED.md`、`API_CONTRACT.md`、`DATA_MODEL.md`、`RELEASE_BASELINE.md`、`CHANGE_REQUEST.md`（如有）等基线文件 | 步骤 2 |
| 步骤 2 | 解析增量需求资料 | 已提取新增/修改/删除需求，并标注差异标记 | 步骤 3 |
| 步骤 3 | 输出变更请求单 | `docs/05-iteration/CHANGE_REQUEST.md` 存在且包含 CR 编号条目 | 步骤 4 |
| 步骤 4 | 执行变更影响分析 | 每条 CR 均已覆盖 7 个影响维度的分析结论 | 步骤 5 |
| 步骤 5 | 输出变更影响分析报告 | `docs/05-iteration/CHANGE_IMPACT.md` 存在且非空 | 步骤 6 |
| 步骤 6 | 提示下一步 | 已向用户输出完成提示与后续建议 | — |

### 默认恢复原则（兜底）

1. 若所有输出文件均不存在，从步骤 1 全量执行。
