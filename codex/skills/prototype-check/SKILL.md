...
12. 无法确认的信息必须标记 `【待确认】`。
13. 基于上下文推断的结论必须标记 `【推断】`。
14. 不得仅以“HTML 可打开”作为通过依据。
15. 后台管理硬门禁：
...
| UX-BLOCK-003 | 编辑闭环缺失 | 编辑弹窗未预填当前行数据，或缺少取消/确认/校验反馈/提交结果反馈任一项 | FAIL（阻塞） |
...
8. 对体验门禁阻塞项执行专项检查：
   ...
   - `UX-BLOCK-003`：检查编辑弹窗是否预填当前行数据，且具备取消、确认、校验反馈、提交结果反馈闭环
