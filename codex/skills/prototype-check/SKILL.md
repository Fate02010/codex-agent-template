...
11. 命中 `UX-BLOCK-001 / UX-BLOCK-002 / UX-BLOCK-003` 任一项时，结论必须为 FAIL，且不允许进入 `dev-implement`。
...
14. 主问题清单中的阻塞项必须填写关联 `UX-TARGET` 与关联 `BUILD-RULE` 字段。
15. 无法确认的信息必须标记 `【待确认】`。
16. 基于上下文推断的结论必须标记 `【推断】`。
17. 不得仅以“HTML 可打开”作为通过依据。
18. 后台管理硬门禁：
...
## 3. 问题清单（按 阻塞 > 重要 > 建议）
| 问题编号 | 问题级别 | 问题类型 | 侧别（display/acceptance） | 页面编号/页面名称 | 断点 | 关联 UX-TARGET | 关联 BUILD-RULE | 问题定位描述 | 问题描述 | 影响 | 建议修复 | 证据 | 是否阻塞进入 dev-implement |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
- 每条问题必须带页面、断点、关联 `UX-TARGET`、关联 `BUILD-RULE`、定位描述与证据。
...
- 命中 `UX-BLOCK-001 / UX-BLOCK-002 / UX-BLOCK-003` 任一项时，必须写明“阻塞，且不允许进入 `dev-implement`”。
