...
11. 交互完整性检查：
   - 验证”必须实现的交互”是否全部实现
   - 验证多级菜单展开/折叠是否正常（点击父级菜单，子菜单展开/折叠）
   - 验证当前页叶子菜单项是否高亮，且所有祖先菜单项（父级、祖父级）是否同步高亮（`BUILD-RULE-016` / `BO-RULE-024`）
   - 验证面包屑导航是否存在（2 级及以上页面）且路径与菜单路径一致（`BUILD-RULE-016` / `BO-RULE-024`）
   - 验证菜单层级不超过 3 级（`BO-RULE-024`）
   - 验证页面跳转是否正常
   - 验证筛选查询/重置是否正常
   - 验证分页点击是否正常
   - 验证新建/编辑弹窗是否正常
...
- 必须优先使用固定阻塞编号输出体验门禁问题：
  ...
  - `UX-BLOCK-011`（分页语义缺失）
  - `UX-BLOCK-012`（分页重置缺失）
  - `BO-RULE-001~024`（后台可冻结门禁 Fail-fast，含多级菜单导航规则）
- 其余问题类型可继续使用：
  - `双轨漂移`
  - `展示过曝`
  - `验收缺失`
...
- 命中任一后台 Fail-fast 门禁项（`BO-RULE-001~024`）时，必须写明”阻塞，且不允许进入 `dev-implement`”。
