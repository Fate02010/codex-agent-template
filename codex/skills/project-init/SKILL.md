...
| 步骤 | 步骤名称 | 完成判定条件 | 续执起点 |
|---|---|---|---|
| 步骤 0 | P0 Gate | `AGENTS.md` 可读，仓库处于无可运行工程骨架或需补齐场景，未输出 BLOCKED | 步骤 1 |
| 步骤 1 | 读取约束并判定场景 | 技术栈约束已读取，项目元数据（groupId/artifactId/projectName/Java 版本）已提取 | 步骤 2 |
| 步骤 2 | 生成后端 Maven 多模块骨架 | `backend/<project-name>-parent/` 目录结构存在，pom.xml 及各子模块目录已生成 | 步骤 3 |
| 步骤 3 | 补齐后端强制配置 | 启动类、Mapper 扫描、MyBatis-Plus 配置、application.yml、error 多语言配置均已补齐 | 步骤 4 |
| 步骤 4 | 生成前端后台骨架 | `frontend/<project-name>/web/` 目录存在，package.json、main.ts、router、stores、api 等文件已生成 | 步骤 5 |
| 步骤 5 | 注释与规范占位 | 生成的 Java 类模板包含 Javadoc 占位，实体字段有注释占位 | 步骤 6 |
| 步骤 6 | 测试目录初始化 | `tests/api/` 目录存在，后端最小 `ApplicationTests` 已创建 | 步骤 7 |
| 步骤 7 | 自检 | 所有自检项通过（后端结构、MyBatis-Plus 接入、前端目录等） | — |
