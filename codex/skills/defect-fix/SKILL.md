...
| 步骤 | 步骤名称 | 完成判定条件 | 续执起点 |
|---|---|---|---|
| 步骤 1 | 读取失败上下文 | 失败用例清单已读取，每个失败用例已关联完整用例信息、需求与接口 | 步骤 2 |
| 步骤 2 | 登记缺陷 | `docs/04-testing/DEFECT_LOG.md` 文件存在且至少有一条缺陷记录 | 步骤 3 |
| 步骤 3 | 根因分析 | `DEFECT_LOG.md` 中每个缺陷的"根因分析"章节已填写 | 步骤 4 |
| 步骤 4 | 文档优先修复 | 涉及设计问题的缺陷已先更新相关文档（或确认为纯代码 Bug 可跳过） | 步骤 5 |
| 步骤 5 | 代码修复 | 每个缺陷的修复代码已提交，且修复针对根因 | 步骤 6 |
| 步骤 6 | 回归测试 | 失败用例已重新运行，所有 P0 用例已执行，回归结果已记录 | 步骤 7 |
| 步骤 7 | 更新缺陷状态 | `DEFECT_LOG.md` 中所有已修复缺陷状态已更新为"已修复"或"已验证" | 步骤 8 |
| 步骤 8 | 追加回归轮次到 TEST_REPORT.md | `docs/04-testing/TEST_REPORT.md` 末尾已追加本轮回归轮次章节 | 步骤 9 |
| 步骤 9 | 循环或完成 | 所有缺陷已验证且 P0 回归全部通过，或已上报用户决策 | 步骤 10 |
| 步骤 10 | 文档同步检查 | 所有文档变更已记录在各文档的变更记录中 | — |
