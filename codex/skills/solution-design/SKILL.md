...
| 步骤 | 步骤名称 | 完成判定条件 | 续执起点 |
|---|---|---|---|
| 步骤 0 | 校验需求基线 | `PRD_RECTIFIED.md` 状态为 `已冻结` 且 precheck 已执行 | 步骤 0.5 |
| 步骤 0.5 | P0 Gate | 三份输出文档路径可用，precheck 报告结论通过 | 步骤 1 |
| 步骤 1 | 初始化输出载体 | `docs/03-architecture/` 目录存在，三份输出文件均已初始化 | 步骤 2 |
| 步骤 2 | 识别系统形态 | 系统形态（单端/多端）已判断，服务命名已确定 | 步骤 3 |
| 步骤 3 | 输出 ARCHITECTURE.md | `docs/03-architecture/ARCHITECTURE.md` 文件存在且结构完整 | 步骤 4 |
| 步骤 4 | 输出 API_CONTRACT.md | `docs/03-architecture/API_CONTRACT.md` 文件存在且结构完整 | 步骤 5 |
| 步骤 5 | 输出 DATA_MODEL.md | `docs/03-architecture/DATA_MODEL.md` 文件存在且结构完整 | 步骤 6 |
| 步骤 6 | 输出追溯矩阵 | `API_CONTRACT.md` 第 9 节追溯矩阵已填写，`ARCHITECTURE.md` 第 10 节引用已标注 | 步骤 7 |
| 步骤 7 | 交叉校验 | 交叉校验检查清单已完成确认 | 步骤 8 |
| 步骤 8 | 提示下一步 | 已完成结果提示 | — |
