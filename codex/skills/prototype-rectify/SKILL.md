...
| 步骤 | 步骤名称 | 完成判定条件 | 续执起点 |
|---|---|---|---|
| 步骤 0 | P0 Gate（阻塞） | `PROTOTYPE_CHECK_REPORT.md` 可读且问题编号可追溯，或已输出 `BLOCKED` 结论 | 步骤 1 |
| 步骤 1 | 解析问题并排序 | 所有问题已按 `阻塞 > 重要 > 建议` 重新排序 | 步骤 2 |
| 步骤 2 | 建立修复映射 | 每个问题的修复映射（问题编号、修改落点、预期结果）已建立 | 步骤 3 |
| 步骤 3 | 执行修复 | 设计文档（`UI_DESIGN_SPEC.md`、`PAGE_FLOW.md` 等）与原型文件（`display/*.html`、`acceptance/*.html` 等）均已同步更新 | 步骤 4 |
| 步骤 4 | 复核与回写 | `docs/02-design/PROTOTYPE_FIX_LOG.md` 存在且包含已关闭与未关闭问题清单，已关闭条目含复检证据 | 步骤 5 |
| 步骤 5 | 输出下一步建议 | 已向用户输出下一步操作建议 | — |
