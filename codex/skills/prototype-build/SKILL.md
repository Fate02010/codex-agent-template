...
15. 必须实现的交互至少包括：
   - 多级菜单展开/折叠（含二级、三级菜单）
   - 菜单展示与高亮联动：当前叶子菜单项高亮，且所有祖先菜单项（父级、祖父级）同步高亮
   - 面包屑导航显示（2 级及以上页面强制，路径与菜单路径一致）
   - 页面跳转与返回路径
   - 按钮可用/禁用与反馈
   - 筛选查询与重置
   - 分页点击与数据更新
...
28. 后台排版结构硬约束（强制）：
   ...
   - `BUILD-RULE-016`：多级菜单导航
     - 菜单结构必须与 `SCREEN_INVENTORY.md` 的导航树一致，禁止自行发明菜单项
     - 当前页面的叶子菜单项及所有祖先菜单项必须同时高亮（通过 JS 动态添加 active/is-active 类）
     - 2 级及以上页面必须渲染 `ElBreadcrumb` 面包屑，路径与菜单路径完全一致
     - 菜单折叠/展开状态必须可交互（点击一级菜单展开/折叠二级；点击二级菜单展开/折叠三级）
     - 菜单层级不得超过 3 级
29. 任一 `BUILD-RULE-010~016` 不满足时，结论必须为 build FAIL 并阻塞进入 `prototype-check`。
...
- 产出 `PROTOTYPE_BUILD_NOTES.md`，至少包含：
  ...
  - 页面清单、资产说明、已知限制
  - `BUILD-RULE-001~016` 结构化自检结果表（页面编号、规则编号、结果 PASS/FAIL、证据位置）
  - `BO-RULE-001~024` 结构化自检结果表（页面编号、规则编号、结果 PASS/FAIL、证据位置）
...
- 检查多级菜单是否满足 `BUILD-RULE-016`：
  - 菜单结构与 `SCREEN_INVENTORY.md` 导航树一致
  - 当前页叶子菜单项及所有祖先菜单项同时高亮
  - 2 级及以上页面已渲染面包屑且路径正确
  - 菜单展开/折叠可交互，层级不超过 3 级
- 检查后台页面是否满足 `BO-RULE-001~024`（含多级菜单导航规则）。
- 将 `BUILD-RULE-001~016` 与 `BO-RULE-001~024` 检查结果写入 `PROTOTYPE_BUILD_NOTES.md` 结构化自检结果表。
