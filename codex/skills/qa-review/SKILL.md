...
| 步骤 1 | 建立需求-测试追溯映射 | `F → TC`、`API → TC`、`角色 → TC` 映射表已在内存中建立 | 步骤 2 |
| 步骤 2 | 逐维度审查 | 10 个评审维度均已检查，发现问题已记录 | 步骤 3 |
| 步骤 3 | 问题分类与定级 | 所有问题已按 🔴/🟠/🟡 分级 | 步骤 4 |
| 步骤 4 | 输出问题清单 | `docs/04-testing/TEST_REVIEW_ISSUES.md` 存在且包含总体结论 | 步骤 5 |
| 步骤 5 | 提示下一步 | 已向用户输出下一步操作建议 | — |
