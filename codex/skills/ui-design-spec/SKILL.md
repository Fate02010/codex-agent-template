...
15. 后台规范消费约束（强制）：
   - 涉及后台管理页面时，必须消费 `BACKOFFICE_UI_SPEC.md` 中 `BO-RULE-001~024`
   ...
   - `UI_REVIEW_CHECKLIST.md` 必须逐项回写 `BO-RULE-001~024` 的可判定检查项
16. 多级菜单导航规则（强制）：
   - 必须从 `PRD_RAW.md` 的 `NAV-TREE`（导航结构分类表）导入菜单结构，禁止自行发明菜单层级
   - `SCREEN_INVENTORY.md` 必须以层级树形式输出导航结构（一级 > 二级 [> 三级]），并为每个页面标注菜单路径
   - `UI_DESIGN_SPEC.md` 必须逐页声明：菜单路径、面包屑路径（2 级及以上强制）
   - `COMPONENT_GUIDELINES.md` 必须定义侧边栏菜单组件的 4 种状态：激活态、折叠态、父级高亮态、禁用态
   - 菜单层级约束必须对齐 `BO-RULE-024`：最多 3 级、祖先高亮、面包屑完整
17. 冷启动规则（强制）：
   - 若目标目录不存在，先创建目录。
...
- 对后台管理页面必须额外声明：
  ...
  - 菜单路径（来自 PRD `NAV-TREE`，格式：`一级 > 二级 [> 三级]`）
  - 面包屑路径（2 级及以上强制，与菜单路径一致）
- 对每个页面必须补充“产品体验目标与指标”：
...
- 对每个页面必须补充”交互实现清单”：
  - 必须实现的交互（如：多级菜单展开/折叠、当前页菜单高亮及祖先项高亮、面包屑导航、页面跳转、按钮点击）
