...
13. 产品体验目标与指标（强制）：
   - 每个页面必须定义关键任务路径，且必须给出页面 Top3 主任务
   - 每个页面必须定义首屏决策信息，明确首屏必须看到什么信息才能做下一步决策
   - 每条关键任务路径必须定义交互效率指标，明确关键任务步数上限
   - 每个危险动作必须定义风险动作防呆策略，至少包含确认、可撤销、危险分级
   - 每个适用页面必须定义默认值与批量操作策略，明确默认值来源、批量入口和批量反馈
14. 冷启动规则（强制）：
   - 若目标目录不存在，先创建目录。
...
- [ ] 每个页面已定义关键任务路径（Top3）、首屏决策信息、关键任务步数上限、风险动作防呆、默认值与批量操作策略（如适用）；任一缺失即阻塞。
