...
| 步骤 | 步骤名称 | 完成判定条件 | 续执起点 |
|---|---|---|---|
| 步骤 1 | 判定执行模式并读取输入 | 执行模式已确定，对应输入文件已读取，`DOC_CHECK_REPORT.md`/`ITERATION_REVIEW.md` 门禁状态已校验（冻结模式） | 步骤 2 |
| 步骤 2 | 确定迭代范围或冻结对象 | 本轮迭代纳入 CR 列表已确定，`CHANGE_REQUEST.md` 的 `纳入迭代` 字段已回写（增量模式） | 步骤 3 |
| 步骤 3 | 输出迭代计划 | `docs/05-iteration/ITERATION_PLAN.md` 存在且内容非模板，包含迭代概览、范围、Sprint 分配摘要 | 步骤 4 |
| 步骤 4 | 冻结版本基线 | `docs/05-iteration/RELEASE_BASELINE.md` 存在且版本号字段有值（仅冻结场景执行） | 步骤 5 |
| 步骤 5 | 追加变更日志 | `docs/05-iteration/CHANGELOG.md` 存在且本轮版本条目已追加 | 步骤 6 |
| 步骤 6 | 提示下一步 | 已输出后续操作提示 | — |
