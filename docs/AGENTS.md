...
- 文档必须可执行，不允许只写空泛章节标题
- `docs/` 预置模板是加速器，不是执行前提
- 生成型 Skill 必须支持冷启动创建文档，不得依赖“模板文件已存在”
- 执行/门禁型 Skill 缺失上游输入时必须输出 `BLOCKED` 并回退，不得补造上游业务结论
...
- 目标目录不存在：先创建目录
- 目标文件不存在：按 Skill 内置最小结构创建完整文档
- 文档状态为`模板`：整文件覆盖为正式产物结构
- 文档状态不为`模板`：按章节标题增量更新，不按章节序号硬编码
