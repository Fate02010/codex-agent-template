...
本规则适用于所有 Skill 的执行中断与恢复场景。Skill 因上下文窗口耗尽、用户中止或工具调用失败等原因中断后，再次调用时必须先执行恢复扫描，跳过已完成步骤，从断点继续推进。

### 14.2 恢复优先级

1. 每个 SKILL.md 的 `## 断点恢复` 章节若声明了自定义检查点或覆盖规则，以 Skill 自身声明为准。
2. Skill 未声明时，使用本章节（14.3 ～ 14.6）的通用默认规则。
3. 用户在对话中显式声明”从头执行”或”忽略断点”时，跳过恢复扫描，全量重新执行。

### 14.3 恢复扫描执行时机

在步骤 0（P0 Gate / 前置校验）主体执行之前，首先执行恢复扫描：

1. 读取本 Skill 的 `## 断点恢复` 章节中的检查点表格。
2. 按表格从最后一步向前逐步扫描，找到”最后一个已完成”的步骤。
3. 输出恢复摘要（格式见 § 14.5 第 3 条），然后跳转到续执起点。
4. 若检查点表格不存在，仅检查最终输出文件是否存在，存在则判定为已完成。

### 14.4 默认完成状态判定标准（按优先级）

| 优先级 | 判定依据 | 适用场景 |
|---|---|---|
| P1 | 步骤对应输出文件存在且内容非空、非模板占位 | 生成型步骤（最强判定） |
| P2 | 输出文档 `状态` 字段为 `草稿`、`已整改` 或 `已冻结` | 有状态字段的正式文档 |
| P3 | Gate Report 存在且 `verdict` 字段有值（PASS/FAIL/BLOCKED 均算完成） | 门禁类步骤 |
| P4 | `*_REVIEW_ISSUES.md` 存在且至少有一条问题记录 | 评审输出步骤 |
| P5 | 工程骨架文件存在（`pom.xml` / `package.json`） | 工程初始化步骤 |

> P3 说明：门禁步骤”写入结论即完成”——即便结论为 FAIL/BLOCKED，恢复时跳过该步骤，但在恢复摘要中标注结论，提示用户是否需要重新评估。

### 14.5 恢复执行注意事项

1. **不重写已冻结文档**：状态为 `已冻结` 的文档在续执中不得被覆盖。
2. **增量续写**：已完成章节保留原内容，仅从续执起点开始写入或更新。
3. **恢复摘要格式**（每次扫描后必须输出）：
   - 有断点：`[断点恢复] 已完成：步骤0~N | 续执起点：步骤M | 依据：<文件路径/状态>`
   - 无断点：`[断点恢复] 未检测到断点，全量执行`
4. **Gate BLOCKED 续执规则**：步骤 0 结论已写入且为 `BLOCKED` 时，续执仍从步骤 0 开始，重新评估阻塞条件是否消除。
5. **幂等保护**：已存在且内容完整的步骤产物只读取不覆盖（冷启动规则中”增量更新”逻辑已覆盖）。

### 14.6 无步骤 Skill 的处理

对于 `dev-implement`、`openspec-*` 等无显式步骤的路由型/工具驱动型 Skill，断点恢复以最终输出文件存在性为判定依据，不设步骤级检查点。
